function CustomStep(arxmodel,order,data,t,FigName)
    [ystep,tstep] = step(arxmodel,t);
    G = dcgain(arxmodel);
    PlotinputChannels = arxmodel.InputName;
    [~,fitper,~] = compare(arxmodel,data)
    ...
    for i = 1:length(G)
        ...
        sgtitle({"\bf Step Responses: "+"\bf ARX (%): "+num2str(fitper),"Order ["+num2str(order)+"]"},'FontSize',14)
    end
